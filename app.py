...
def predict_next_word(model,tokenizer,text,max_sequence_length):
    ...
    if len(token_list)>max_sequence_length:
        token_list=token_list[-(max_sequence_length-1):] #Ensure that the sequence length matches the max_sequence length
...
#streamlit app
st.title("Predicting the next word with LSTM")
